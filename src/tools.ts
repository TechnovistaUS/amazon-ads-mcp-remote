...
export function registerAllTools(server: McpServer): void {
  ...
  server.registerTool(
    ...
    async () => {
      const data = await adsRequest<unknown>("GET", "/v2/profiles", "");
      return { content: [{ type: "text" as const, text: okText(data) }] };
    }
  );
  ...
  server.registerTool(
    ...
    async ({ profileId, stateFilter, campaignType, count, startIndex }) => {
      const params: Record<string, string | number | boolean> = { count, startIndex };
      if (stateFilter) params.stateFilter = stateFilter;
      if (campaignType) params.campaignType = campaignType;
      const data = await adsRequest<unknown>("GET", "/v2/campaigns", profileId, undefined, params);
      return { content: [{ type: "text" as const, text: okText(data) }] };
    }
  );
  ...
  server.registerTool(
    ...
    async ({ profileId, campaignId, startDate, endDate }) => {
      ...
      const data = await adsRequest<unknown>("POST", "/v2/sp/campaigns/report", profileId, body);
      return { content: [{ type: "text" as const, text: okText(data) }] };
    }
  );
  ...
  server.registerTool(
    ...
    async ({ profileId, name, dailyBudget, startDate, endDate, targetingType, state, premiumBidAdjustment }) => {
      const body: Record<string, unknown> = { name, campaignType: "sponsoredProducts", targetingType, state, dailyBudget, startDate, premiumBidAdjustment };
      if (endDate) body.endDate = endDate;
      const data = await adsRequest<unknown>("POST", "/v2/sp/campaigns", profileId, [body]);
      return { content: [{ type: "text" as const, text: okText(data) }] };
    }
  );
  ...
  server.registerTool(
    ...
    async ({ profileId, campaignId, name, state, dailyBudget, endDate }) => {
      ...
      const data = await adsRequest<unknown>("PUT", "/v2/sp/campaigns", profileId, [body]);
      return { content: [{ type: "text" as const, text: okText(data) }] };
    }
  );

  server.registerTool(
    "amazon_ads_get_account_info",
    {
    ...
    async ({ profileId }) => {
      const data = await adsRequest<unknown>("GET", `/v2/profiles/${profileId}`, profileId);
      return { content: [{ type: "text" as const, text: okText(data) }] };
    }
  );

  server.registerTool(
    "amazon_ads_get_token_status",
    {
    ...
    async () => {
      ...
      return { content: [{ type: "text" as const, text: okText(status) }] };
    }
  );

  server.registerTool(
    "amazon_ads_set_bidding_strategy",
    {
    ...
    async ({ profileId, campaignId, strategy }) => {
      const data = await adsRequest<unknown>("PUT", "/v2/sp/campaigns", profileId, [{ campaignId, bidding: { strategy } }]);
      return { content: [{ type: "text" as const, text: okText(data) }] };
    }
  );

  server.registerTool(
    "amazon_ads_apply_bid_multiplier",
    {
    ...
    async ({ profileId, campaignId, multiplier, adGroupId }) => {
      const params: Record<string, string | number | boolean> = { campaignId };
      if (adGroupId) params.adGroupId = adGroupId;
      const keywords = await adsRequest<Array<{ keywordId: string; bid: number }>>("GET", "/v2/sp/keywords", profileId, undefined, params);
      if (!Array.isArray(keywords) || keywords.length === 0) {
        return { content: [{ type: "text" as const, text: JSON.stringify({ message: "No keywords found", campaignId }) }] };
      }
      ...
      const data = await adsRequest<unknown>("PUT", "/v2/sp/keywords", profileId, updates);
      return { content: [{ type: "text" as const, text: okText({ updated: updates.length, multiplier, data }) }] };
    }
  );

  server.registerTool(
    "amazon_ads_create_bid_rule",
    {
    ...
    async ({ campaignId, name, condition, adjustment, priority }) => {
      ...
      return { content: [{ type: "text" as const, text: okText(rule) }] };
    }
  );

  server.registerTool(
    "amazon_ads_get_bid_rules",
    {
      title: "Get Bid Rules",
    ...
    async ({ campaignId }) => {
      ...
      return { content: [{ type: "text" as const, text: okText(rules) }] };
    }
  );

  server.registerTool(
    "amazon_ads_adjust_bid",
    {
      title: "Adjust Bid",
    ...
    async ({ profileId, campaignId, keywordId, newBid, reason }) => {
      const existing = await adsRequest<Record<string, unknown> & { bid?: number }>("GET", `/v2/sp/keywords/${keywordId}`, profileId);
      const previousBid = existing.bid ?? 0;
      const data = await adsRequest<unknown>("PUT", "/v2/sp/keywords", profileId, [{ keywordId, bid: newBid }]);
      recordBidAdjustment({ campaignId, previousBid, newBid, reason });
      return { content: [{ type: "text" as const, text: okText({ keywordId, previousBid, newBid, reason, result: data }) }] };
    }
  );

  server.registerTool(
    "amazon_ads_set_target_acos",
    {
    ...
    async ({ campaignId, targetAcos }) => {
      ...
      return { content: [{ type: "text" as const, text: okText({ targetAcos, campaignId, rulesCreated: [highRule, lowRule], message: `Target ACoS of ${targetAcos}% set.` }) }] };
    }
  );

  server.registerTool(
    "amazon_ads_create_bid_automation",
    {
    ...
    async ({ campaignId, targetAcos, ruleIds }) => {
      ...
      return { content: [{ type: "text" as const, text: okText(automation) }] };
    }
  );

  server.registerTool(
    "amazon_ads_update_bid_rule",
    {
    ...
    async ({ ruleId, enabled, condition, adjustment, priority, name }) => {
      const updates: Parameters<typeof updateBidRule>[1] = {};
      if (enabled !== undefined) updates.enabled = enabled;
      if (condition !== undefined) updates.condition = condition;
      if (adjustment !== undefined) updates.adjustment = adjustment;
      if (priority !== undefined) updates.priority = priority;
      if (name !== undefined) updates.name = name;
      const updated = updateBidRule(ruleId, updates);
      if (!updated) {
        return { content: [{ type: "text" as const, text: JSON.stringify({ error: `Rule ${ruleId} not found` }) }] };
      }
      return { content: [{ type: "text" as const, text: okText(updated) }] };
    }
  );

  server.registerTool(
    "amazon_ads_get_adjustment_history",
    {
    ...
    async ({ campaignId }) => {
      ...
      return { content: [{ type: "text" as const, text: okText(history) }] };
    }
  );

  server.registerTool(
    "amazon_ads_get_bid_automation",
    {
    ...
    async ({ automationId, campaignId }) => {
      if (automationId) {
        ...
        return { content: [{ type: "text" as const, text: okText(automation ?? { error: `Automation ${automationId} not found` }) }] };
      }
      const automations = getBidAutomations(campaignId);
      return { content: [{ type: "text" as const, text: okText(automations) }] };
    }
  );

  server.registerTool(
    "amazon_ads_delete_bid_rule",
    {
    ...
    async ({ ruleId }) => {
      ...
      return { content: [{ type: "text" as const, text: okText({ ruleId, deleted, message: deleted ? "Rule deleted." : `Rule ${ruleId} not found.` }) }] };
    }
  );
}
